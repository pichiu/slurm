...
對於重疊預約的支援非常有限，有兩種特定的操作模式可用。為了便於系統維護，您可以建立具有「maint」旗標的預約，與現有預約重疊。這允許管理員輕鬆地為整個叢集建立維護預約，而無需移除或重新排程既有的預約。請求存取這些既有預約之一的使用者將被阻止使用也在此維護預約中的資源。例如，使用者 alan 和 brenda 可能每天從中午到下午 1 點有一些節點的預約。如果所有節點從下午 12:30 開始有維護預約，他們在預約中可能啟動的唯一作業必須在維護預約開始的 12:30 前完成。

第二個例外以與維護預約相同的方式運作，只是它不會在會計系統中記錄為維護保留的節點。它需要在建立第二個預約時使用「overlap」旗標。這可用於確保在群組預約中為特定使用者提供資源的可用性。使用先前 alan 和 brenda 有 10 節點 60 分鐘預約的範例，我們可能想要在時間段的前 30 分鐘為 brenda 保留其中的 4 個節點。在這種情況下，建立一個重疊預約（總共兩個預約）可能比建立三個單獨的預約更簡單，部分原因是使用任何預約都需要作業指定預約名稱。

1. 為 alan 和 brenda 提供持續完整 60 分鐘的六節點預約
2. 為 brenda 提供前 30 分鐘的四節點預約
3. 為 alan 和 brenda 提供最後 30 分鐘的四節點預約

如果在建立預約時使用「maint」或「overlap」旗標，可以在預約中建立預約，再在第三個預約中建立。請注意，具有「maint」或「overlap」旗標的預約不會被同樣具有「maint」或「overlap」旗標的後續預約移除資源，因此預約的嵌套僅適用於兩層深度。
